                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-end">
                                ...
                                <li class="breadcrumb-item active" aria-current="page"> <i class="fas fa-folder-open me-1"></i> Projects</li>
                            </ol>
                        </div>
                    </div>
                    <!--end::Row-->
...
$(document).ready(function(){
   ...
   $('#projectsTable').DataTable({
            ...
            error:function(xhr){
                if(xhr.status === 422){
                    ...
                    if(errors.description){
                        $('#projectDescriptionError').text(errors.description[0]).show();
    ...
    $(document).on('click', '.edit-projects', function(){
        ...
        $.ajax({
            ...
            if (response.logo) {
                $('#editLogoPreview').attr("src", `/storage/${response.logo}`).show();
            } else {
   ...
   $('#editProjectForm').on('submit', function(e){
    ...
    $('#projectNameEditError, #projectDescriptionEditError, #projectImageError').text('').hide();
    ...
    $.ajax({
        ...
        if (xhr.status === 422) {
            ...
            if (errors.name) {
                $('#projectNameEditError').text(errors.name[0]).show(); // Set and show the error message
                $('#edit_project_name').addClass('is-invalid');
            }

            if (errors.description) {
                $('#projectDescriptionEditError').text(errors.description[0]).show();
